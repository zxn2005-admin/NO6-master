	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('yishengyuyue','通知')" @click="jiuzhentongzhionAcross('通知','是','','','')" type="warning">通知</el-button>
					<el-button class="approval_btn" v-if="btnAuth('yishengyuyue','审核')" type="warning" @click="approvalClick()">审核</el-button>
...
<script setup>
	...
	const approvalClick = (btnType='审核') => {
		...
		if(!btnAuth('yishengyuyue',btnType)){
	...
	const jiuzhentongzhionAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		...
		nextTick(()=>{
			router.push(`/index/jiuzhentongzhiAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
